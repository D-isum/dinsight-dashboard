...
const hasPersistedBaselineSelection = (
  config: AppliedWearTrendConfig | DraftWearTrendConfig | null
) =>
  Boolean(
    config &&
    (config.baselineClusterValues.length > 0 ||
      (config.baselineRange?.start && config.baselineRange?.end))
  );
...
interface HealthInsightsWorkspaceProps {
  controlsMode?: 'managed' | 'hidden';
  controlsPresentation?: 'inline' | 'drawer';
...
export function HealthInsightsWorkspace({
  ...
  const [activeControlTab, setActiveControlTab] = useState<'baseline' | 'thresholds'>('baseline');
  ...
  const skipNextUiPrefsPersistRef = useRef(true);
  ...
  useEffect(() => {
    ...
    skipNextUiPrefsPersistRef.current = true;
  ...
  useEffect(() => {
    ...
    if (skipNextUiPrefsPersistRef.current) {
      skipNextUiPrefsPersistRef.current = false;
      return;
    }
    ...
    window.dispatchEvent(
      new CustomEvent(INSIGHTS_DISTANCE_THRESHOLD_CONFIG_EVENT, {
        detail: distanceThresholdConfig,
      })
    );
  }, [
    activePlotTab,
  ...
  const { data: streamingStatus } = useQuery<StreamingStatus | null>({
    queryKey: ['streaming-status', datasetId],
    enabled: !!datasetId,
    queryFn: async () => {
      if (!datasetId) {
        return null;
  ...
  useEffect(() => {
    ...
    const localAppliedForDataset = localConfig?.datasetId === datasetId ? localConfig : null;
    const localDraftForDataset =
      localDraftConfig?.datasetId === datasetId ? localDraftConfig : null;
    const resolvedLocal = hasPersistedBaselineSelection(localDraftForDataset)
      ? localDraftForDataset
      : (localAppliedForDataset ?? localDraftForDataset);
    if (!resolvedLocal) {
    ...
    const localAppliedSource = localAppliedForDataset ?? resolvedLocal;
    setAppliedClusterSignature(
      JSON.stringify(Array.from(new Set(localAppliedSource.baselineClusterValues)).sort())
    );
    setAppliedRangeSignature(
      JSON.stringify({
        start: localAppliedSource.baselineRange?.start ?? '',
        end: localAppliedSource.baselineRange?.end ?? '',
      })
    );
    setAppliedIncludeMonitoring(localAppliedSource.includeMonitoring);

    if (localAppliedForDataset) {
      setHasAppliedWearTrendRun(true);
      setLastWearTrendRunAt(localAppliedForDataset.appliedAt);
    }
    hasHydratedPersistedConfigRef.current = true;
  }, [datasetId, userId]);
  ...
  useEffect(() => {
    ...
    const resolvedDraftForDataset = resolvedDraft?.datasetId === datasetId ? resolvedDraft : null;
    const resolvedAppliedForDataset =
      resolvedApplied?.datasetId === datasetId ? resolvedApplied : null;

    const resolved = hasPersistedBaselineSelection(resolvedDraftForDataset)
      ? resolvedDraftForDataset
      : (resolvedAppliedForDataset ?? resolvedDraftForDataset);
    if (!resolved) {
      return;
    }
    skipNextDatasetMetadataResetRef.current = true;
    ...
    const appliedSource = resolvedAppliedForDataset ?? resolved;
    const resolvedClusterSignature = JSON.stringify(
      Array.from(new Set(appliedSource.baselineClusterValues)).sort()
    );
    const resolvedRangeSignature = JSON.stringify({
      start: appliedSource.baselineRange?.start ?? '',
      end: appliedSource.baselineRange?.end ?? '',
    });
    setAppliedClusterSignature(resolvedClusterSignature);
    setAppliedRangeSignature(resolvedRangeSignature);
    setAppliedIncludeMonitoring(appliedSource.includeMonitoring);
    if (resolvedAppliedForDataset) {
      setHasAppliedWearTrendRun(true);
      setLastWearTrendRunAt(resolvedAppliedForDataset.appliedAt);
      writeScoped(
        INSIGHTS_APPLIED_WEAR_CONFIG_KEY,
        userId,
        JSON.stringify(resolvedAppliedForDataset)
      );
      window.dispatchEvent(new CustomEvent(INSIGHTS_APPLIED_WEAR_CONFIG_EVENT));
    }

    if (resolvedDraftForDataset) {
      writeScoped(INSIGHTS_DRAFT_WEAR_CONFIG_KEY, userId, JSON.stringify(resolvedDraftForDataset));
    }
  ...
  useEffect(() => {
    if (!hasHydratedPersistedConfigRef.current || !datasetId || !metadataColumn) {
      return;
    }
  ...
  const shouldRenderWearPlots =
    Boolean(wearResult) && hasAppliedWearTrendRun && isBaselineSelectionApplied;
  ...
  const distanceEChart = useMemo(() => {
    ...
    const option: EChartsOption = {
      ...
      xAxis: {
        type: 'value',
        name: wearResult.metadata_column,
        nameLocation: 'middle',
        nameGap: 46,
        min: xAxisRange?.[0],
        max: xAxisRange?.[1],
  ...
  useEffect(() => {
    if (!publishHealthSnapshot || !wearResult || !latestMonitoringInterval) {
      return;
    }
  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        {!controlsAreHidden && (
          <Card
            id="asset-monitor-analysis-controls"
            className={cn(
            ...
            aria-label={controlsPresentation === 'drawer' ? t('insights.controls') : undefined}
          >
            ...
            >
                ...
                <div className="space-y-3 rounded-lg border border-input bg-muted/20 p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {streamingStatus?.is_active ? t('common.active') : t('insights.idle')}
                      </p>
                    </div>
                  </div>
                  {(hasPendingChanges ||
              ...
              <Tabs
                ...
                onValueChange={(value) =>
                  setActiveControlTab(value === 'thresholds' ? 'thresholds' : 'baseline')
                }
              >
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="baseline">{t('insights.controlTabBaseline')}</TabsTrigger>
                  <TabsTrigger value="thresholds">{t('insights.controlTabThresholds')}</TabsTrigger>
                </TabsList>
              </Tabs>

              {activeControlTab === 'baseline' && (
                <>
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      disabled={
                        !metadataColumnsQuery.data || metadataColumnsQuery.data.length === 0
                      }
                    >
                  ...
                  <div className="space-y-2 rounded-lg border border-input p-3">
                    ...
                    {filteredBaselineIntervalValues.length > BASELINE_CLUSTER_PAGE_SIZE && (
                      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
                        <span>
                          {t('insights.paginationShowing', {
                            from: clusterPageStartIndex + 1,
                            to: Math.min(
                              currentClusterPage * BASELINE_CLUSTER_PAGE_SIZE,
                              filteredBaselineIntervalValues.length
                            ),
                            total: filteredBaselineIntervalValues.length,
                          })}
                        </span>
              ...
              {activeControlTab === 'baseline' && (
                <div
                  className={cn(
                    ...
                    controlsPresentation === 'drawer' &&
                      'sticky bottom-0 z-10 shadow-[0_-10px_24px_-16px_rgba(0,0,0,0.45)]'
                  )}
                >
        ...
        <div className="min-w-0 space-y-5">
          ...
          <Card
            className={cn('border-border/60', embedded && 'border-0 bg-transparent shadow-none')}
          >
            {!hideWorkspaceHeader && (
            ...
            <CardContent className={cn('space-y-4', embedded && 'p-0')}>
              {!metadataColumn ? (
                ...
                <>
                  {isUpdatingWearTrend && (
                    <p className="text-xs text-muted-foreground">{t('insights.updatingChart')}</p>
                  )}
                  {!compactChart && showWearSummaryMetrics && (
                    <div className="space-y-3 rounded-lg border border-input p-3">
                              ...
                              <div className="flex items-center justify-between text-sm">
                                ...
                                <div className="flex gap-2">
                                  <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                      setTransitionPage((page) => Math.max(1, page - 1))
                                    }
