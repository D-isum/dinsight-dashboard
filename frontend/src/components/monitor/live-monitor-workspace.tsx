...
const LIVE_RENDER_DENSITY: Record<RenderDensity, { maxPoints: number; shortMaxPoints: string }> = {
  fast: { maxPoints: 20_000, shortMaxPoints: '20k' },
  balanced: { maxPoints: 50_000, shortMaxPoints: '50k' },
  detailed: { maxPoints: 100_000, shortMaxPoints: '100k' },
};
...
export type MonitorWorkspaceControls = 'managed' | 'hidden';
export type MonitorControlsPresentation = 'inline' | 'drawer';

interface LiveMonitorWorkspaceProps {
  controlsMode?: MonitorWorkspaceControls;
  controlsPresentation?: MonitorControlsPresentation;
  ...
  healthStatusOverride?: MachineHealthResult | null;
  publishHealthSnapshot?: boolean;
}

export function LiveMonitorWorkspace({
  ...
  publishHealthSnapshot = true,
}: LiveMonitorWorkspaceProps = {}) {
  const { user } = useAuth();
  ...
  const [activeControlTab, setActiveControlTab] = useState<'live' | 'healthy' | 'display'>('live');
  ...
  const effectiveRefreshIntervalMs = isStreaming ? refreshIntervalMs : LIVE_IDLE_STATUS_REFRESH_MS;
  const controlsAreCollapsed = controlsOpen == null ? isControlsCollapsed : controlsOpen === false;
  const controlsAreHidden = controlsMode === 'hidden' || controlsAreCollapsed;
  const hideControls = () => {
    if (controlsOpen == null) setIsControlsCollapsed(true);
    onControlsOpenChange?.(false);
  };
  const showControls = () => {
    if (controlsOpen == null) setIsControlsCollapsed(false);
    onControlsOpenChange?.(true);
  };
  const chartHeightClass = compactChart
    ? 'h-[clamp(420px,56vh,620px)]'
    : 'h-[clamp(560px,74vh,800px)]';
  const chartHeight = compactChart ? 'clamp(420px, 56vh, 620px)' : 'clamp(560px, 74vh, 800px)';
  ...
  const renderDensityLabel = (density: RenderDensity) =>
    density === 'fast'
      ? t('live.renderDensityFast')
      : density === 'balanced'
        ? t('live.renderDensityBalanced')
        : t('live.renderDensityDetailed');
  ...
  useEffect(() => {
    ...
    // Query refreshes can return the snapshot that preceded a recent local save.
    // Reapplying it would erase the active dataset boundary and its classification.
    if (localUpdatedAt > 0 && !isNewerThanLocal) {
      return;
    }
  ...
  const anomalyMachineStatus = useMachineHealthStatus({ anomalyPercentage, wearTrendScore: null });
  const machineStatus = healthStatusOverride ?? anomalyMachineStatus;
  ...
  const undoLastBoundary = () => setBoundaries((current) => current.slice(0, -1));
  ...
  return (
    <div className="space-y-5">
        ...
        <Card className="border-warning-border bg-warning-bg ">
          <CardContent className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm">
            <p className="text-warning-text ">
              {t('live.remoteSettingsDetected', {
                date: prefsConflict.updatedAt
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={keepLocalPreferences}>
                {t('live.keepLocalSettings')}
              </Button>
              <Button size="sm" onClick={applyRemotePreferences}>
                {t('live.applyRemoteSettings')}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      >
        {!controlsAreHidden && controlsPresentation === 'drawer' && (
          <button
            type="button"
            className="fixed inset-x-0 bottom-0 top-16 z-40 cursor-default bg-black/25 backdrop-blur-[1px]"
        ...
        {!controlsAreHidden && (
          <Card
            id="asset-monitor-map-controls"
            className={cn(
              'min-w-0 border-border/60',
              controlsPresentation === 'inline' &&
            ...
            aria-label={controlsPresentation === 'drawer' ? t('live.controls') : undefined}
          >
            <CardHeader className="border-b border-border/70 pb-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <Button
                  variant="outline"
                  size={controlsPresentation === 'drawer' ? 'icon-sm' : 'sm'}
                  onClick={hideControls}
                  autoFocus={controlsPresentation === 'drawer'}
                  className={cn('shrink-0', controlsPresentation === 'inline' && 'gap-2')}
                  aria-label={
                    controlsPresentation === 'drawer'
                      ? t('live.closeControls')
                      : t('live.hideControls')
                  }
                  title={
                    controlsPresentation === 'drawer'
            ...
            <CardContent
              className={cn(
                'space-y-4 py-4',
                controlsPresentation === 'inline' &&
                  'lg:max-h-[calc(100vh-13rem)] lg:overflow-y-auto',
                controlsPresentation === 'drawer' && 'max-h-[calc(100vh-9.75rem)] overflow-y-auto'
              )}
            >
              {controlsPresentation === 'inline' && (
                <div
                  className={cn('rounded-lg border p-3 text-sm', stateTone[machineStatus.state])}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
              ...
              <Tabs
                ...
                onValueChange={(value) =>
                  setActiveControlTab(
                    value === 'healthy' ? 'healthy' : value === 'display' ? 'display' : 'live'
                  )
                }
              >
                <TabsList className="grid w-full grid-cols-3">
                  <TabsTrigger value="live">{t('live.controlTabLive')}</TabsTrigger>
                  <TabsTrigger value="healthy">{t('live.controlTabHealthy')}</TabsTrigger>
                  <TabsTrigger value="display">{t('live.controlTabDisplay')}</TabsTrigger>
                </TabsList>
              </Tabs>
                ...
                <>
                  ...
                  <div className="space-y-2 rounded-md border border-input p-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {isStreaming
                        ? t('live.refreshEvery', {
                            seconds: effectiveRefreshIntervalMs / 1000,
                          })
                        : t('live.idleRefreshEvery', {
                            seconds: effectiveRefreshIntervalMs / 1000,
                          })}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 rounded-md border border-input p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge variant="outline">
                        {t('live.renderDensityPoints', {
                          count: LIVE_RENDER_DENSITY[renderDensity].shortMaxPoints,
                        })}
                      </Badge>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      {(Object.keys(LIVE_RENDER_DENSITY) as RenderDensity[]).map((density) => (
                        ...
                        >
                          {renderDensityLabel(density)}
                        </Button>
                      ))}
                    </div>
                    <p className="text-xs text-muted-foreground">{t('live.renderDensityHint')}</p>
                  </div>
                </>
              )}
              ...
              {activeControlTab === 'live' && (
                <>
                  <div className="rounded-lg border border-input p-3 text-sm space-y-2">
                    ...
                    <Progress
                      value={streamingStatus?.progress_percentage ?? 0}
                      className="w-full"
                    />
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">
                          {lastStatusAt ? formatTime(lastStatusAt) : '-'}
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">
                          {autoRefresh ? `${effectiveRefreshIntervalMs / 1000}s` : t('live.paused')}
                        </p>
                      </div>
                    </div>
                    {streamingStatusError && (
                      <p className="rounded-md border border-warning-border bg-warning-bg px-2 py-1 text-xs text-warning-text">
                        {t('live.statusRetrying', { message: streamingStatusError.message })}
                      </p>
                    )}
                  </div>
                </>
                ...
                <>
                  ...
                  {manualSelectionEnabled && (
                    <div className="space-y-3 rounded-lg border border-input p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-medium">{t('live.healthyRegionShape')}</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <span className="self-center text-xs text-muted-foreground">
                          {t('live.regionCount', { count: boundaries.length })}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  {boundaries.length > 0 && (
                    <div className="space-y-2 rounded-lg border border-input p-3">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-medium">{t('live.healthyOperatingRegions')}</p>
                        <Badge variant="outline">
                          {t('live.regionCount', { count: boundaries.length })}
                        </Badge>
                      </div>
                      <div className="grid gap-1.5">
                        {boundaries.map((boundary, index) => (
                          <div
                            key={boundary.id}
                            className="flex min-w-0 items-center justify-between gap-2 rounded-md border border-border bg-surface-muted/40 px-2 py-1"
                          >
              ...
              {activeControlTab === 'healthy' && manualClassification && (
        ...
        <div className="min-w-0 space-y-5">
          ...
          >
            {!hideWorkspaceHeader && (
              <CardHeader className={cn(embedded && 'px-0 pb-3 pt-0')}>
            ...
            <CardContent className={cn(embedded && 'p-0')}>
              ...
              {!compactChart && controlsPresentation !== 'drawer' && boundaries.length > 0 && (
                <div
                  className={cn(
                    'mb-4 space-y-2 rounded-lg border border-input p-3',
                    embedded && 'mb-3 flex flex-wrap items-center gap-2 space-y-0 p-2'
                  )}
                >
                  <p className="text-sm font-medium">{t('live.healthyOperatingRegions')}</p>
                  <div className={cn('space-y-2', embedded && 'flex flex-wrap gap-2 space-y-0')}>
                    {boundaries.map((boundary, index) => (
                      <div
                        key={boundary.id}
                        className={cn(
                          'flex items-center justify-between text-sm',
                          embedded && 'gap-2 rounded-md border border-border px-2 py-1'
                        )}
                      >
                        <span>
                          {t('live.areaLabel', {
              ...
              <ChartFrame
                ...
                description={
                  followLatest
                    ? t('live.followLatestDescription')
                ...
                stats={
                  <>
                    ...
                    {!compactChart && (
                      <ChartStat
                        label={t('live.sampleCap')}
                        value={formatNumber(renderMaxPoints)}
                      />
                    )}
                    <ChartStat
                  ...
                  <>
                    ...
                    {manualSelectionEnabled && (
                      <select
                        ...
                        className="h-9 rounded-md border border-input bg-background px-2 text-xs text-fg"
                        aria-label={t('live.healthyRegionShape')}
                        title={selectionHelper(selectionMode)}
                      >
                        {SELECTION_MODE_ORDER.map((mode) => (
                    ...
                    <Button
                      ...
                      variant={manualSelectionEnabled ? 'default' : 'outline'}
                      onClick={() => {
                        setManualSelectionEnabled((current) => !current);
                        setAnomalyResult(null);
                      }}
                    >
                      <MousePointerClick className="mr-2 h-4 w-4" aria-hidden="true" />
                    ...
                    <Button
                      ...
                      onClick={undoLastBoundary}
                      disabled={boundaries.length === 0}
                      aria-label={t('live.undoLastRegion')}
                      title={t('live.undoLastRegion')}
                    >
                    ...
                    <Button
                      ...
                      onClick={clearBoundaries}
                      disabled={boundaries.length === 0}
                      aria-label={t('live.clearRegions')}
                      title={t('live.clearRegions')}
                    >
              ...
              >
                {isLoadingBaseline || (selectedId && isLoadingMonitoring) ? (
                  ...
                  <>
                    ...
                    />
                    {(manualSelectionEnabled || showContours) && (
                      <p className="border-t border-border px-2 py-1 text-xs text-muted-foreground">
                        {manualSelectionEnabled
                          ...
                          : ''}
                        {showContours
                          ? `${manualSelectionEnabled ? ' ' : ''}${t(
                              'live.baselineDensityOverlay'
                            )}`
                          : ''}
                      </p>
                    )}
      ...
      <AlertDialog open={isResetConfirmOpen} onOpenChange={setIsResetConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('live.resetStreamStateTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('live.resetStreamStateDescription', { id: selectedId ?? '-' })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {t('live.confirmResetStreamState')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
